# Fresh Translation Full Bible Logos/Proofreading Files

Generated files:

- `fresh_translation_full_bible_logos_bible.docx`: Logos Personal Book source. Compile as resource type `Bible`.
- `fresh_translation_full_bible_reference_notes.docx`: Combined Logos Personal Book source with OT milestones remapped to standard English/MT references where possible and NT milestones left on their standard references. Compile as resource type `Bible`.
- `fresh_translation_full_bible_proofreading.docx`: clean proofreading/printing copy without Logos milestone or field syntax.
- `fresh_translation_full_bible_diagnostics.json`: build counts and cross-reference/note diagnostics.
- `fresh_translation_full_bible_preview.md`: quick preview sample for spot-checking.
...
3. Click `Add book`.
4. Set `Type` to `Bible`.
5. Add `fresh_translation_full_bible_logos_bible.docx` as the body file.
6. Build the book.
7. If Logos exposes advanced datatype/index settings, keep the source milestones on `Bible`.

Reference-note bridge import:

`fresh_translation_full_bible_reference_notes.docx` remaps OT milestones to standard English/MT references where a reliable mapping is available. NT TR source rows already use standard NT versification.

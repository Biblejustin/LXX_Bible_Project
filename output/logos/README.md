...
- Cross-references: omitted from the default Personal Book build. This keeps the source from being flooded by TSK/OpenBible footnotes and leaves translation, textual, supplemental, and name-meaning notes visible. Rebuild with `--include-crossrefs` only when a cross-reference-heavy edition is desired.

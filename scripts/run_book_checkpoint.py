...
from fresh_bible.pipeline_common import ROOT, SCRIPTS, count_csv_rows, load_csv, run_cmd
...
def main() -> None:
    ...
    if not args.skip_contextual_name_decisions:
        run_cmd([py, str(SCRIPTS / "apply_contextual_proper_name_decisions.py"), "--summary-only"])
    if not args.skip_sync:
        run_cmd([py, str(SCRIPTS / "sync_ot_support_text.py")])
    if not args.skip_name_notes:
        run_cmd([py, str(SCRIPTS / "update_proper_name_transliteration_notes.py"), "--summary-only"])
    run_cmd(
        [
            py,
    ...
    if args.smoke_test:
        run_cmd([py, "-m", "pytest", "-q", "tests/test_smoke.py"])
        ...
        json.dumps(
            {
                ...
                "decision_rows": count_csv_rows(DECISIONS),
                "footnote_rows": count_csv_rows(FOOTNOTES),
